...
public class RequestReader implements Request {
  ...
  private void authorize(User user, Door door) {
    if (user == null) {
      ...
      Area fromSpace = door.getSpaceFrom();
      Area toSpace = door.getSpaceTo();
      authorized = user.canDoAction(this.action)   //what
        && user.canBeInSpace(fromSpace) //where
          && user.canBeInSpace(toSpace) //where
          && user.canSendRequests(now); //when


      if(!user.canSendRequests(now)){
        addReason("Present date and/or time " + now + "not in schedule for user " + user.getName());
       }
    }
  }
}
